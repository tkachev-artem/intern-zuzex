...
export const HomeNavbar = () => {
  ...
  // открываем модальное окно для создания поста
  const handleOpenPostModal = () => {
    setModalOpen(true);
  };

  // закрываем модальное окно для создания поста
    ...
    <Navbar.Root className="navbar-root">
      <Navbar.Content className="navbar-content">
        <Navbar.Item className="navbar-tabs">
          <Button variant="ghost" size="lg" aria-label="Главная" onClick={() => void navigate('/')}>
              <FaReact size={24}/>
              <SiRedux size={24}/>
              ИТ ЛЕНТА 
            </Button>
